const jhonData = {
    "name": "HIDROPLAST",
    "name2": "HIDROPLAST",
    "role": "Šta je Hidroplast",
    "ptext": "Projektovanje i izvođenje",
    "aboutme": "O Nama",
    "aboutdetails": "Hidroplast je firma koja se bavi projektovanjem hidrotehničkih instalacija kao i njihovim izvođenjem. Poslovanje firme je orijentisano prvenstveno prema industriji i stanogradnji. Izrada instalacija vodovoda i kanalizacije što u stambenim jedinicama tako i u industrijskim objektima iziskuje nova rešenja a samim tim i nove tehnologije. Zahvaljujući modernizaciji objekata raznih namena bili smo u prilici da implementiramo nove i pouzdane vodovodne i kanalizacione sisteme. ",
    "service":[
        {
          "id" : 1,
          "iconSource": "/icons/pencil.png",
          "iconClass": "icon-desktop",
          "serviceTitle": "Projektovanje instalacija",
          "sDetails" : "Projektovanje hidrotehničkih instalacija u industrijskim kao i u stambeno-poslovnim objektima."
        },
        {
          "id" : 2,
          "iconSource": "/icons/plug.png",
          "serviceTitle": "Elektrofuziono zavarivanje",
          "sDetails" : "Zavarivanje cevi i fitinga niskog i visokog pritiska. Vrsta materijala za zavarivanje: HDPE, PP, PPR."
        },
        {
          "id" : 3,
          "iconSource": "/icons/wrench.png",
          "serviceTitle": "Kompresiono spajanje",
          "sDetails" : "Spajanje cevi i fitinga vodovoda i kanalizacije. Vrste materijala: PE, PEX, PVC, HTTP, PVC KG, PP."
        },
        {
          "id" : 4,
          "iconSource": "/icons/water.png",
          "serviceTitle": "Livenarija",
          "sDetails" : "Mehaničko spajanje i montaža vodovodnih fazonskih komada koji su izrađeni od duktilnog liva."
        },
        {
          "id" : 5,
          "iconSource": "/icons/pipes.png",
          "serviceTitle": "Cinkovana instalacija",
          "sDetails" : "Izrada sanitarne i PP instalacije od cinkovanih cevi i fitinga. Narezivanje cevi se vrši mehanički."
        },
        {
          "id" : 6,
          "iconSource": "/icons/glue.png",
          "serviceTitle": "Sistem lepljenja",
          "sDetails" : "Izrada kompletne instalacije vodovoda sistemom lepljenja cevi i fazonskih komada."
        }
    ],
    "contact":[
      {
        "id": "1",
        "title": "Kontakt",
        menuItems: [
          {
            id: 1,
            icon: 'icon_pin_alt',
            text: 'Adresa: Maršala Tita 65, 24309 Mali Beograd',
          },
          {
            id: 2,
            icon: 'icon_phone',
            text: 'Telefon: +381 63 1287665',
          },
          {
            id: 3,
            icon: 'icon_mail',
            text: 'Email: hidroplast89@gmail.com',
          },
          {
            id: 4,
            icon: 'icon_globe',
            text: 'Sajt: www.hidroplast.rs',
          }
        ],
        "contentTitle": "Još nešto o nama",
        "content": "Hidroplast je osnovan 2014. godine sa idejom da se bavi izradom instalacija u stanogradnji i industriji. Od samog početka obavljanja delatnosti razvoj je usmeravan ka usavršavanju što pripremnih radova tako i izvođačkih. Posle šest nepunih godina rada i razvoja Hidroplast je kompanija koja je uspela u svom prvobitnom cilju. Danas se bavimo projektovanjem i izvođenjem instalacija i iza sebe imamo završene projekte raznih namena, a to se odnosi na stambene, poslovne, ugostiteljske i industrijske objekte. ",
      },
    ],
  contacttitle: 'Pišite',
}

export default jhonData;

